...
export def main [
    ...
    --san: list<string> = [],  # Optional additional SAN entries (e.g., ["DNS:idp.internal","IP:127.0.0.1"])
    --days: int = 36500,
    --keep-intermediate,
    ...
    # Read CA name from shared CA metadata
    ...
    if not (($ca_cert | path exists) and ($ca_key | path exists)) {
        error make {msg: $"CA files not found at ($shared_ca_dir)/($ca_name).{{crt,key}}. Please run scripts/tls/generate-ca.nu first"}
    }
