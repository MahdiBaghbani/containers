...
def main [--verbose] {
    ...
    # ------------------------------------------------------------------
    # Routed CLI / Make smoke tests (help, dispatch, passthrough)
    #
    # These exercise operator-facing paths through scripts/dockypody.nu and the
    # Makefile, not just low-level helpers. They are non-Docker and CI-safe: the
    # help paths only print, the make path is `-n` (dry run), and the forced-CA
    # path is hermetic (stubbed git + openssl, temp repo) so it never writes a
    # real CA.
    # ------------------------------------------------------------------
