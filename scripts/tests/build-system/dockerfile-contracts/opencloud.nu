...
# opencloud Dockerfile contract, override script, and manifest tests.
...
export def opencloud-tests [verbose: bool] {
  [
    (run-test "Test 39v: Dockerfile drift - opencloud clone-source contract and SHA checkout guard" {
      assert-opencloud-dockerfile-clone-source-contract "services/opencloud/Dockerfile.alpine"
      true
    } $verbose)
    (run-test "Test 39w: opencloud override scripts use clone-source.nu contract" {
    ...
    (run-test "Test 39y: Dockerfile drift - opencloud web/reva override script COPY and RUN wiring" {
      assert-opencloud-dockerfile-override-wiring "services/opencloud/Dockerfile.alpine"
      true
    } $verbose)
    (run-test "Test 39x: Real manifest - opencloud v6.1.0 and main alpine OPENCLOUD_REF_KIND build args" {
      let svc = "opencloud"
      let platform = "alpine"
