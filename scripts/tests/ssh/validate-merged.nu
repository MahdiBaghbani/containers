...
# validate-ssh-config-merged valid/disabled/invalid cases.

use ../../lib/validate/ssh.nu [validate-ssh-config-merged]
use ../lib.nu [run-test]

export def validate-merged-tests [verbose: bool] {
    [
        (run-test "validate-ssh-config-merged: valid merged config" {
            let merged = { ssh: { enabled: true, mode: "server", default_user: "root", port: 22 } }
            validate-ssh-config-merged $merged "test-service"
            true
        } $verbose)
        (run-test "validate-ssh-config-merged: disabled SSH valid" {
            let merged = { ssh: { enabled: false, mode: "disabled", default_user: "root", port: 22 } }
            validate-ssh-config-merged $merged "test-service"
            true
        } $verbose)
        (run-test "validate-ssh-config-merged: invalid merged config rejected" {
