...
3. Open `https://nextcloud1.docker/` in another tab.
4. Open `https://nextcloud2.docker/` in another tab.
5. Keep mitmweb open and confirm you see flows for:
   - `nextcloud1.docker`
   - `nextcloud2.docker`

Notes:

- Firefox is configured (via `.env` and `user.js`) to use `mitmproxy.docker:8080`
  as its HTTP and HTTPS proxy by default.
- `mitmproxy.docker` is excluded from the browser proxy so mitmweb does not
  self-proxy.
...
other over the shared network.

Run from this directory:

```bash
docker exec nextcloud1.docker sh -lc 'curl -fsS -x http://mitmproxy.docker:8080 https://nextcloud2.docker/.well-known/ocm | head -c 200'
```
