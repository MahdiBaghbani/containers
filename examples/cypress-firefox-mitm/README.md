...
- `cypress-firefox.docker` can open `https://cypress-mitmproxy.docker/`
- `cypress-runner.docker` can open the same URL in Chrome inside the Cypress desktop
...
4. Open Cypress at `https://localhost:5900`, then open Chrome inside it and
   browse to `https://cypress-mitmproxy.docker/` and compare behavior.
