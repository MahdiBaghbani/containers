...
# Initialize share providers configuration
...
export def init_shareproviders [] {
  print "Initializing share providers configuration..."
  ...
  if not $has_config {
    print $"Share providers config not found - copying and templating from image..."
    
    # Copy share providers config template
    let source_config = $"($CONFIG_DIR)/($config_file)"
    if not ($source_config | path exists) {
      error make { msg: $"Share providers config template not found: ($source_config)" }
    }
    ^cp $source_config $config_path
    
    # Copy all JSON files (users, groups, providers, etc.) if needed by share providers config
  ...
  mut placeholder_map = {
    ...
    "grpc-address": $":($shareproviders_grpc_port)"
