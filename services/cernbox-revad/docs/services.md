...
**Container Name:** `cernbox-1-test-revad-authprovider-ocmsharecode`
**Port:** 9280 (gRPC)
**Config File:** `authprovider-ocmsharecode.toml` (provided by revad-base)

See [Reva Base Services - OCM Share Code Auth Provider](../../revad-base/docs/services.md#ocm-share-code-auth-provider) for service description.

### OCM Exchanged Token Auth Provider

**Container Name:** `cernbox-1-test-revad-authprovider-ocmexchangedtoken`
**Port:** 9282 (gRPC)
**Config File:** `authprovider-ocmexchangedtoken.toml` (provided by revad-base)

See [Reva Base Services - OCM Exchanged Token Auth Provider](../../revad-base/docs/services.md#ocm-exchanged-token-auth-provider) for service description.
...
- **Compose ordering:** The example stack starts `gateway` after `idp`, then
  starts all other Reva services after `gateway`, while `web` also waits for
  `idp` plus the three dataproviders
