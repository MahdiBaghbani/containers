...
export def run_path [hook_name: string, user: string] {
  ...
  print $"=> Searching for hook scripts \(*.sh\) to run, located in the folder \"($hook_folder)\""
  ...
  # Find all .sh scripts in hook folder
  let scripts = (ls $hook_folder | where type == file and name =~ '\.sh$' | sort-by name)
  ...
  for script in $scripts {
    ...
    # Run script as appropriate user
    let result = (run_as $user $script_path | complete)
