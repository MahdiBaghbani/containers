...
def main [...cmd_args: string] {
  ...
  # Run if: command starts with "apache", is "php-fpm", or NEXTCLOUD_UPDATE=1
  ...
  if (version_greater $image_version $installed_version) {
    ...
    if $installed_version != "0.0.0.0" {
      # Check for major version jump (not allowed)
  ...
  # Step 6: Update htaccess if requested
...
# Check for differences between image config files and installed config files
