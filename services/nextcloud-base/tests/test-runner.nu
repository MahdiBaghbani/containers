...
def main [
  --suite: string = "all"       # Which test suite to run (all, utils, nextcloud-init, entrypoint, source-prep, apache-config, redis-config, hooks, post-install, seeded-users, managed-config)
  ...
  let test_suites = if $suite == "all" {
    ["utils", "nextcloud-init", "entrypoint", "source-prep", "apache-config", "redis-config", "hooks", "post-install", "seeded-users", "managed-config"]
  } else {
    [$suite]
  }
