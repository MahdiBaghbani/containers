...
function pull_profile (){
	...
	0|1)
		...
		if [[ $DEBUG == true ]]; then
			OUTPUT=$(http_proxy="" https_proxy="" /usr/bin/kasm-profile-sync --download "${HOME}" --insecure --remote ${KASM_API_HOST} --port ${KASM_API_PORT} -c ${KASM_PROFILE_CHUNK_SIZE} --token ${KASM_API_JWT} --verbose 2>&1 )
		else
			OUTPUT=$(http_proxy="" https_proxy="" /usr/bin/kasm-profile-sync --download "${HOME}" --insecure --remote ${KASM_API_HOST} --port ${KASM_API_PORT} -c ${KASM_PROFILE_CHUNK_SIZE} --token ${KASM_API_JWT} 2>&1 )
		fi
...
function cleanup () {
    ...
    if [ -n "${KASM_PROCS['kasmvnc']:-}" ]; then
        vncserver -kill $DISPLAY >/dev/null 2>&1 || true
    fi

    for process in "${!KASM_PROCS[@]}"; do
        pid="${KASM_PROCS[$process]}"
        if [ -n "$pid" ] && kill -0 "$pid" >/dev/null 2>&1; then
            kill -s SIGTERM "$pid" >/dev/null 2>&1 || true
        fi
    done
    ...
    for process in "${!KASM_PROCS[@]}"; do
        pid="${KASM_PROCS[$process]}"
        if [ -n "$pid" ] && kill -0 "$pid" >/dev/null 2>&1; then
            kill -s SIGKILL "$pid" >/dev/null 2>&1 || true
        fi
    done
...
function start_kasmvnc (){
	...
	local vnc_options="${VNCOPTIONS:-}"
	local auth_options=""
	local clipboard_options=""
	...
	case "${OCM_KASM_AUTH_MODE:-none}" in
		none)
			auth_options="-DisableBasicAuth -SecurityTypes=None"
			;;
		password)
			auth_options=""
			;;
		*)
			log "Unsupported OCM_KASM_AUTH_MODE: ${OCM_KASM_AUTH_MODE}" "ERROR"
			exit 1
			;;
	esac

	case "${OCM_KASM_CLIPBOARD_MODE:-default}" in
		default)
			;;
		disabled)
			clipboard_options="-SendCutText=0 -AcceptCutText=0"
			;;
		send-only)
			clipboard_options="-AcceptCutText=0"
			;;
		receive-only)
			clipboard_options="-SendCutText=0"
			;;
		*)
			log "Unsupported OCM_KASM_CLIPBOARD_MODE: ${OCM_KASM_CLIPBOARD_MODE}" "ERROR"
			exit 1
			;;
	esac

	vnc_options="$vnc_options -select-de manual"

	if [[ ${KASM_SVC_PRINTER:-1} == 1 ]]; then
		vnc_options="$vnc_options -UnixRelay printer:/tmp/printer"
	fi

	if [[ ${KASM_SVC_SMARTCARD:-1} == 1 ]]; then
		vnc_options="$vnc_options -UnixRelay smartcard:/tmp/smartcard"
	fi

	if [[ "${BUILD_ARCH}" =~ ^aarch64$ ]] && [[ -f /lib/aarch64-linux-gnu/libgcc_s.so.1 ]] ; then
		LD_PRELOAD=/lib/aarch64-linux-gnu/libgcc_s.so.1 vncserver $DISPLAY $KASMVNC_HW3D -drinode $DRINODE -websocketPort $NO_VNC_PORT -httpd ${KASM_VNC_PATH}/www -interface 0.0.0.0 -BlacklistThreshold=0 -FreeKeyMappings $vnc_options $auth_options $clipboard_options
	else
		vncserver $DISPLAY $KASMVNC_HW3D -drinode $DRINODE -websocketPort $NO_VNC_PORT -httpd ${KASM_VNC_PATH}/www -interface 0.0.0.0 -BlacklistThreshold=0 -FreeKeyMappings $vnc_options $auth_options $clipboard_options
	fi
...
function start_audio_out_websocket (){
	if [[ ${KASM_SVC_AUDIO:-1} == 1 ]]; then
		...
		$STARTUPDIR/jsmpeg/kasm_audio_out-linux kasmaudio 8081 4901 ${HOME}/.vnc/self.pem ${HOME}/.vnc/self.pem "ocm_user:$VNC_PW"  &
...
function start_audio_in (){
	if [[ ${KASM_SVC_AUDIO_INPUT:-1} == 1 ]]; then
		...
		$STARTUPDIR/audio_input/kasm_audio_input_server --ssl --auth-token "ocm_user:$VNC_PW" --cert ${HOME}/.vnc/self.pem --certkey ${HOME}/.vnc/self.pem &
...
function start_upload (){
	if [[ ${KASM_SVC_UPLOADS:-1} == 1 ]]; then
		...
		$STARTUPDIR/upload_server/kasm_upload_server --ssl --auth-token "ocm_user:$VNC_PW" --port 4902 --upload_dir ${HOME}/Uploads &
...
function start_gamepad (){
	if [[ ${KASM_SVC_GAMEPAD:-1} == 1 ]]; then
		...
		$STARTUPDIR/gamepad/kasm_gamepad_server --ssl --auth-token "ocm_user:$VNC_PW" --cert ${HOME}/.vnc/self.pem --certkey ${HOME}/.vnc/self.pem &
...
nu "$STARTUPDIR/render-kasmvnc-config.nu"

# first entry is control, second is view (if only one is valid for both)
mkdir -p "$HOME/.vnc"
PASSWD_PATH="$HOME/.kasmpasswd"
PRIMARY_ACCESS="${OCM_KASM_PRIMARY_ACCESS:-write}"
EXTRA_VIEWER="${OCM_KASM_EXTRA_VIEWER:-0}"
...
case "${OCM_KASM_AUTH_MODE:-none}" in
    none)
        echo -e "\n---------  creating passwordless KasmVNC users  ---------"
        ;;
    password)
        echo -e "\n---------  creating password-protected KasmVNC users  ---------"
        ;;
    *)
        log "Unsupported OCM_KASM_AUTH_MODE: ${OCM_KASM_AUTH_MODE}" "ERROR"
        exit 1
...
case "$PRIMARY_ACCESS" in
    write)
        echo -e "${VNC_PW}\n${VNC_PW}\n" | kasmvncpasswd -u ocm_user -wo
        ;;
    view)
        echo -e "${VNC_PW}\n${VNC_PW}\n" | kasmvncpasswd -u ocm_user -r
        log "OCM_KASM_PRIMARY_ACCESS=view. KasmVNC will start without a write-capable user." "WARNING"
        ;;
    none)
        log "OCM_KASM_PRIMARY_ACCESS=none. No primary KasmVNC user will be created." "WARNING"
        ;;
    *)
        log "Unsupported OCM_KASM_PRIMARY_ACCESS: $PRIMARY_ACCESS" "ERROR"
        exit 1
        ;;
esac

case "$EXTRA_VIEWER" in
    1|true|yes)
        echo -e "${VNC_VIEW_ONLY_PW}\n${VNC_VIEW_ONLY_PW}\n" | kasmvncpasswd -u ocm_viewer -r
        ;;
    0|false|no)
        ;;
    *)
        log "Unsupported OCM_KASM_EXTRA_VIEWER: $EXTRA_VIEWER" "ERROR"
        exit 1
        ;;
esac

if [[ "$PRIMARY_ACCESS" == "none" ]] && [[ "$EXTRA_VIEWER" =~ ^(0|false|no)$ ]]; then
    log "OCM_KASM_PRIMARY_ACCESS=none requires OCM_KASM_EXTRA_VIEWER=1 to create at least one KasmVNC user." "ERROR"
    exit 1
fi
...
# tail vncserver logs (opt-in; default quiet)
if [ "${OCM_KASM_TAIL_VNC_LOG:-0}" = "1" ]; then
    tail -f $HOME/.vnc/*$DISPLAY.log &
    KASM_PROCS['vnc_log_tail']=$!
fi
